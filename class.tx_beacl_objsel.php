	...
	function select($PA, $fobj) {
		...
		switch ($PA['row']['type']) {
			...
			case '0':
				...
				if (!$GLOBALS['BE_USER']->isAdmin()) $items = \TYPO3\CMS\Backend\Utility\BackendUtility::blindUserNames($items, $BE_USER->userGroupsUID, 1);
			...
			case '1':
				...
				if (!$GLOBALS['BE_USER']->isAdmin()) $items = \TYPO3\CMS\Backend\Utility\BackendUtility::blindGroupNames($items, $BE_USER->userGroupsUID, 1);
