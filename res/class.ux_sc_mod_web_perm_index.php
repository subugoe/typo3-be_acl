 ...
 */
class ux_SC_mod_web_perm_index extends \TYPO3\CMS\Perm\Controller\PermissionModuleController {
	...
	function notEdit() {
		...
		if (!$GLOBALS['BE_USER']->isAdmin()) $be_user_Array = \TYPO3\CMS\Backend\Utility\BackendUtility::blindUserNames($be_user_Array, $groupArray, 0);
		$be_group_Array = \TYPO3\CMS\Backend\Utility\BackendUtility::getGroupNames();
		if (!$GLOBALS['BE_USER']->isAdmin()) $be_group_Array = \TYPO3\CMS\Backend\Utility\BackendUtility::blindGroupNames($be_group_Array, $groupArray, 0);
		...
		foreach ($tree->tree as $data) {
			...
			$groupN = \TYPO3\CMS\Core\Utility\GeneralUtility::fixed_lgd_cs($groupN, 20);
			...
			// First column:
			$cells[] = '
					<td align="left" nowrap="nowrap"' . $bgCol . $this->generateTitleAttribute($data['row']['uid'], $be_user_Array, $be_group_Array) . '>' . $data['HTML'] . htmlspecialchars(\TYPO3\CMS\Core\Utility\GeneralUtility::fixed_lgd_cs($data['row']['title'], $tLen)) . '&nbsp;</td>';
			...
			if ($this->MOD_SETTINGS['mode'] == 'perms') {
				...
				// ACL rows
				if (!empty($aclUsers)) {
					$cells[] = '<td' . $bgCol . '><img' . \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH, 'gfx/line.gif', 'width="5" height="16"') . ' alt="" /></td><td' . $bgCol . '>' . $this->countAcls($this->aclList[$data['row']['uid']][0]) . '</td>';
					foreach ($aclUsers as $uid) {
						$tmpBg = $bgCol;
						...
						$cells[] = '<td' . $bgCol . '><img' . \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH, 'gfx/line.gif', 'width="5" height="16"') . ' alt="" /></td>
						<td' . $tmpBg . ' nowrap="nowrap">' . ($data['row']['uid'] ? ' ' . $this->printPerms($this->aclList[$data['row']['uid']][0][$uid]['permissions']) : '') . '</td>';
					}
				}
				if (!empty($aclGroups)) {
					$cells[] = '<td' . $bgCol . '><img' . \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH, 'gfx/line.gif', 'width="5" height="16"') . ' alt="" /></td><td' . $bgCol . '>' . $this->countAcls($this->aclList[$data['row']['uid']][1]) . '</td>';
					foreach ($aclGroups as $uid) {
						$tmpBg = $bgCol;
		...
		if ($this->getTypo3Version() >= 4004000)
			$icon = '<img' . \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH, 'gfx/legend.gif', 'width="86" height="75"') . ' alt="" />';
		else
		...
		if ($this->getTypo3Version() >= 4004000) {
			...
			$code .= '<br /><br />' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('status-status-permission-granted') . ': ' . $LANG->getLL('A_Granted', 1);
			$code .= '<br />' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('status-status-permission-denied') . ': ' . $LANG->getLL('A_Denied', 1);
		} else {
	...
	function acl_objectSelector($type, &$displayPointer, $conf) {
		...
		if (!empty($aclObjects)) {
			...
			$currentSelectionOverride_raw = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP('tx_beacl_objsel');
			...
			// display selector
			$displayCode = '<select size="' . (class_exists('\TYPO3\CMS\Core\Utility\MathUtility') ? \TYPO3\CMS\Core\Utility\MathUtility::forceIntegerInRange(count($aclObjects), 5, 15) : \TYPO3\CMS\Core\Utility\GeneralUtility::forceIntegerInRange(count($aclObjects), 5, 15)) . '" name="tx_beacl_objsel[' . $type . '][]" multiple="multiple">';
	...
	function buildACLtree($users, $groups) {
		...
		$rootLine = \TYPO3\CMS\Backend\Utility\BackendUtility::BEgetRootLine($this->id);
	...
	function printTableHeader($cells) {
		global $BACK_PATH;
		$verticalDivider = '<td class="bgColor2"><img' . \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH, 'gfx/line.gif', 'width="5" height="16"') . ' alt="" /></td>';
		$wrappedCells = Array();
	...
	function printPerms($int) {
		...
		if ($this->getTypo3Version() >= 4004000) {
			...
			foreach ($permissions as $permission) {
				if ($int & $permission) {
					$str .= \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('status-status-permission-granted', array('tag' => 'a', 'title' => $LANG->getLL($permission, 1), 'onclick' => 'WebPermissions.setPermissions(' . $pageId . ', ' . $permission . ', \'delete\', \'' . $who . '\', ' . $int . ');'));
				} else {
					$str .= \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('status-status-permission-denied', array('tag' => 'a', 'title' => $LANG->getLL($permission, 1), 'onclick' => 'WebPermissions.setPermissions(' . $pageId . ', ' . $permission . ', \'add\', \'' . $who . '\', ' . $int . ');'));
				}
			}
